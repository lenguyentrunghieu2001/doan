...
io.on("connection", (socket) => {
  ...
  socket.on("user_mess", (data) => {
    ...
    io.sockets
      .to(user[data.userGet])
      .emit("user_mess", `user ${data.userSend} has joined`);
  });
  ...
  socket.on("comments", async (data) => {
    console.log(data);
    let comment = await socketHandler.addCommentFig(
      data.user,
      data.figure,
      data.title
    );
...
app.get("/video_api", async (req, res) => {
  const result = [];
  let data = await cloudinary.v2.search
    .expression("folder:video/*")
    .sort_by("public_id", "desc")
    .max_results(30)
    .execute();
  data.resources.forEach((video) =>
    result.push({ url: video.url, name: video.filename })
  );
  res.json(result);
});
